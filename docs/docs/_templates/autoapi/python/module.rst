{% import 'module_summary.rst' as macros %}
{% if obj.display %}
   {% if is_own_page %}
{{ obj.id.replace('_', ' ').capitalize().replace(' ', ' ', 1).title() }}
{{ "=" * obj.id|length }}
...
.. autoapisummary::
   ...
   {{ exception.id }}
            ...
            {% set all_classes = visible_children|selectattr("type", "equalto", "class")|list %}
{{ macros.auto_summary(all_classes) }}
