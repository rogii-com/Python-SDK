...
def test_set_project(solo_client_project):
    assert solo_client_project is not None
    assert solo_client_project.to_dict()['name'] == PROJECT_NAME
    assert solo_client_project.to_df().at[0, 'name'] == PROJECT_NAME


def test_get_project_wells(solo_client_project):
    assert solo_client_project.wells.to_dict()
    assert not solo_client_project.wells.to_df().empty


def test_get_well(solo_client_project):
    well = solo_client_project.wells.find_by_name(WELL_NAME)
...
def test_get_well_trajectory(solo_client_project):
    trajectory = solo_client_project.wells.find_by_name(WELL_NAME).trajectory

    assert trajectory.to_dict()
    assert not trajectory.to_df().empty


def test_get_well_trajectory_point(solo_client_project):
    trajectory_point = solo_client_project.wells.find_by_name(WELL_NAME).trajectory.find_by_md(0)
...
def test_get_well_interpretations(solo_client_project):
    interpretations = solo_client_project.wells.find_by_name(WELL_NAME).interpretations

    assert interpretations.to_dict()
    assert not interpretations.to_df().empty


def test_get_well_interpretation(solo_client_project):
    interpretation = solo_client_project.wells.find_by_name(WELL_NAME).interpretations.find_by_name(
        INTERPRETATION_NAME
    )
...
def test_get_well_starred_interpretation(solo_client_project):
    starred_interpretation = solo_client_project.wells.find_by_name(WELL_NAME).starred_interpretation
...
def test_get_well_target_lines(solo_client_project):
    target_lines = solo_client_project.wells.find_by_name(WELL_NAME).target_lines

    assert target_lines.to_dict()
    assert not target_lines.to_df().empty


def test_get_well_target_line(solo_client_project):
    target_line = solo_client_project.wells.find_by_name(WELL_NAME).target_lines.find_by_name(TARGET_LINE_NAME)
...
def test_get_well_starred_target_line(solo_client_project):
    starred_target_line = solo_client_project.wells.find_by_name(WELL_NAME).starred_target_line
...
def test_get_well_nested_wells(solo_client_project):
    nested_wells = solo_client_project.wells.find_by_name(WELL_NAME).nested_wells

    assert nested_wells.to_dict()
    assert not nested_wells.to_df().empty


def test_get_well_nested_well(solo_client_project):
    nested_well = solo_client_project.wells.find_by_name(WELL_NAME).nested_wells.find_by_name(NESTED_WELL_NAME)
...
def test_get_well_starred_nested_well(solo_client_project):
    starred_nested_well = solo_client_project.wells.find_by_name(WELL_NAME).starred_nested_well
