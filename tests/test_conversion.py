...
def test_get_converted_meter_trajectory(project):
    ...
    assert not trajectory_df.query('md == 0 & incl == 0 & azim == 0').empty
...
def test_get_converted_meter_horizon(project):
    ...
    assert np_is_close(horizon_data_df.query('md == 11288')['tvd'], 11517.76902768946)
    assert np_is_close(horizon_data_df.query('md == 13770')['tvd'], 11462.009984212762)
    assert np_is_close(horizon_data_df.query('md == 16854')['tvd'], 11263.199107699302)
...
def test_get_converted_foot_horizon(ft_project):
    ...
    assert not horizon_data_df.query('md == 0 & tvd.isnull()').empty
    assert not horizon_data_df[(np_is_close(horizon_data_df['md'], 10196.85039))]['tvd'].isnull().empty
    assert not horizon_data_df[(np_is_close(horizon_data_df['md'], 20134.51444))]['tvd'].isnull().empty
    assert not horizon_data_df[(np_is_close(horizon_data_df['md'], 30026.24672))]['tvd'].isnull().empty

    assert np_is_close(horizon_data_df[(np_is_close(horizon_data_df['md'], 37034.12073))]['tvd'], 37787.95935)
    assert np_is_close(horizon_data_df[(np_is_close(horizon_data_df['md'], 39714.56693))]['tvd'], 37685.00777)
    assert np_is_close(horizon_data_df[(np_is_close(horizon_data_df['md'], 55295.27559))]['tvd'], 36952.75298)
